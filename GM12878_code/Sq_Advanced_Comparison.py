...
# 评估模型
model1.load_weights('SPEID_gm12878_seq_SMC.h5')
...
# 评估模型
model2.load_weights('new_gm12878_sq_SMC.h5')
...
X_en_test = np.load('GM12878_anchor1_ts.npy')
X_pr_test = np.load('GM12878_anchor2_ts.npy')
y_test = np.load('GM12878_labels_ts.npy')
...
checkpointer = ModelCheckpoint(filepath='EPIHilbert.h5', verbose=1, save_best_only=True)



# 评估模型
model3.load_weights('EPIHilbert.h5')
...
model_path = 'saved_models/gm12878_gradient_boosting_model.pkl'
...
# 加载测试数据
anchors1_data_test = np.load('gm12878_features_test_1.npy')
anchors2_data_test = np.load('gm12878_features_test_2.npy')
test_labels = np.load('gm12878_labels_test_1.npy')
...
knn_predictor = joblib.load("gm12878_knn_predictor.pkl")
xgb_predictor = joblib.load("gm12878_xgb_predictor.pkl")
lgb_predictor = joblib.load("gm12878_lgb_predictor.pkl")
lr_predictor = joblib.load("gm12878_lr_predictor.pkl")
...
# # 为每个模型创建单独的ROC和PR曲线数据文件
# models = ['SPEID', 'UniChrom', 'EPI_Hilbert', 'Targetfinder', 'Stacking']
...
#     filename = f'{model}_performance.txt'
#     with open(filename, 'w') as f:
#         # 写入模型名称和总体性能指标
#         f.write(f'{model} Model Performance Summary\n')
#         f.write('=' * (len(model) + 23) + '\n\n')
...
#             f.write(f'FPR: {fprs[i][j]:.4f}, TPR: {tprs[i][j]:.4f}\n')
        
#         f.write('\nPR Curve Data:\n')
#         f.write('--------------\n')
#         for j in range(len(precisions[i])):
#             f.write(f'Precision: {precisions[i][j]:.4f}, Recall: {recalls[i][j]:.4f}\n')
